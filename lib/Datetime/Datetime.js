...
function genOptions(value, props) {
  ...
  }), getDates(value[0], value[1]), locale[props.locale].noon, makeRange(0, 12), makeRange(0, 23), makeRange(0, 59), makeRange(0, 59), getDaysByYear(value[0]), getDaysByYear(value[0])];
...
function getSlotFormattedValue(value, props) {
  ...
  // 数据格式化
  var ret = Slot.formatDataValue([].concat(options), [].concat(currentValue));
...
var Datetime = function (_React$Component) {
  ...
  function Datetime(props) {
    ...
    // 数据格式化
    var ret = Slot.formatDataValue([].concat(_this.options), [].concat(currentValue));
  ...
  _createClass(Datetime, [{
    ...
    value: function setValue(val, confirm, props) {
      ...
      var _Slot$formatDataValue = Slot.formatDataValue(options, val),
    ...
    value: function formatText(arr, text) {
      ...
      for (var i = 0; i < arr.length; i++) {
        var el = arr[i];
        formatArray.push(isArray(el) ? this.formatText(el, locale[localeCode].surfix[colFlags[i]]) : {
          text: addZero(el.text) + (isUndefined(text) ? locale[localeCode].surfix[colFlags[i]] : text),
          value: el.value
        });
      }
      return formatArray;
    ...
    value: function handleChange(value, column) {
      ...
      if (value.length > 2) {
        ...
        if (shouldChangeOptions) {
          ...
          var ret = Slot.formatDataValue(t.options, currentValue);
...
Datetime.propTypes = {
  ...
  value: React.PropTypes.oneOfType([React.PropTypes.number, React.PropTypes.string, React.PropTypes.object]),
